...
#define MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES_DEFAULT    -1
#define MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES_MAX        10000
#define MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES()   (PG_HAS_OPCLASS_OPTIONS() ? \
          ((MSPGIST_MULTIRANGE_Options *) PG_GET_OPCLASS_OPTIONS())->max_ranges : \
          MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES_DEFAULT)
...
} MSPGIST_MULTIRANGE_EXTRACT_Options;
...
PG_FUNCTION_INFO_V1(mspg_range_quad_inner_consistent);
/*
 * SP-GiST consistent function for inner nodes: check which nodes are
 * consistent with given set of queries.
 */
Datum
mspg_range_quad_inner_consistent(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(multirange_mspgist_extract_options);
/**
 * ME-SP-GiST extract options for multirange types
 */
PGDLLEXPORT Datum
multirange_mspgist_extract_options(PG_FUNCTION_ARGS)
{
  local_relopts *relopts = (local_relopts *) PG_GETARG_POINTER(0);

  init_local_reloptions(relopts, sizeof(MSPGIST_MULTIRANGE_EXTRACT_Options));
  add_local_int_reloption(relopts, "max_ranges",
              "number of ranges for extract method",
              MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES_DEFAULT, 1, 
              MSPGIST_MULTIRANGE_EXTRACT_MAX_RANGES_MAX,
              offsetof(MSPGIST_MULTIRANGE_EXTRACT_Options, max_ranges));
...
/**
 * ME-SP-GiST compress function for multirange types
 */
PGDLLEXPORT Datum
multirange_mspgist_extract(PG_FUNCTION_ARGS)
{
  ...
  {
    MSPGIST_MULTIRANGE_EXTRACT_Options *options = 
      (MSPGIST_MULTIRANGE_EXTRACT_Options *) PG_GET_OPCLASS_OPTIONS();
